...
def plotSamplesAndBezierCurves(left10, xControlPoints, yControlPoints, indexesIterated, samplesIterated):
    ...
    # Plot Bezier curves
    for i, (xCtrl, yCtrl, x, y) in enumerate(zip(xControlPoints, yControlPoints, indexesIterated, samplesIterated)):
        ...
        for j in range(len(xCtrl) - 1):
            ax.plot([xCtrl[j], xCtrl[j+1]], [yCtrl[j], yCtrl[j+1]], color='lightgray', linestyle='--')
    ...
    ax.scatter(np.arange(len(left10)), left10, color='black', label='Samples')
